...
<body class="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-black flex">
    ...
    <main class="flex-1 p-8 flex flex-col">
        <div class="flex-1 bg-gray-900/50 border border-gray-800 backdrop-blur-sm mb-6 rounded p-6 flex flex-col relative overflow-hidden">
            ...
            <div id="stickerPreview" class="absolute inset-0 flex items-center justify-center pointer-events-none text-8xl drop-shadow-2xl"></div>
            <canvas id="canvas" class="hidden"></canvas>
            <div class="absolute inset-0 bg-gradient-to-t from-black/10 via-transparent to-black/5 pointer-events-none rounded-xl"></div>
        </div>

        <div class="flex items-center justify-center gap-4">
            <button id="startCameraBtn" class="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white border-0 px-8 py-3 rounded flex items-center gap-2">
                <i class="fas fa-camera"></i> Start Camera
            </button>

            <button id="captureBtn" class="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white border-0 px-8 py-3 rounded flex items-center gap-2 hidden">
                <i class="fas fa-bolt"></i> Capture
            </button>

            <div class="relative">
                <input id="uploadInput" type="file" accept="image/*" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                <button class="border border-gray-600 text-gray-300 hover:bg-gray-800 hover:text-white px-6 py-3 rounded flex items-center gap-2">
                    <i class="fas fa-upload"></i> Upload
                </button>
            </div>

            <button id="retakeBtn" class="border border-gray-600 text-gray-300 hover:bg-gray-800 hover:text-white px-6 py-3 rounded flex items-center gap-2 hidden">
                <i class="fas fa-rotate-left"></i> Retake
            </button>

            <button id="downloadBtn" class="bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white border-0 px-6 py-3 rounded flex items-center gap-2 hidden">
                <i class="fas fa-download"></i> Download
            </button>

            <button id="shareBtn" class="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white border-0 px-8 py-3 rounded hidden">
                Share to Gallery
            </button>
        </div>
    </main>
