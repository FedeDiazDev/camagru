...
<body class="text-gray-300">
    ...
    <div class="w-full max-w-md bg-gray-900/80 border border-gray-800 backdrop-blur-xl rounded-2xl shadow-xl p-6 text-center">
      ...
      <div id="success" class="hidden space-y-6">
        ...
        <div class="space-y-3">
          <a
            href="/login"
            class="block w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white text-center py-3 rounded-md font-medium transition">Sign In to Your Account</a>
          <a
            href="/"
            class="block w-full border border-gray-600 text-gray-300 hover:bg-gray-800 hover:text-white py-3 rounded-md text-center font-medium transition">Explore the Gallery</a>
        </div>
      </div>
  ...
  <script>
    const result = JSON.parse('<?php echo addslashes($jsonResult); ?>');
    ...
    setTimeout(() => {
      ...
      if (result.res) {
        ...
        messageContainer.innerHTML = `<div class="bg-green-500/10 border border-green-500/20 rounded-lg p-4 text-left">
